...
namespace Admin.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        {
            var aaa = db.Users.Where(x => x.Id == id).First();

            aaa.IsDeleted = true;
            db.SaveChanges();

            //return View(db.Users.Where(x => x.Id == id).First());
            return View("AllUsers", db.Users.Where(x => x.IsDeleted == false).ToList());
        }

        [HttpPost]
        public virtual ActionResult DeleteUser(ApplicationUser user)
        {
            try
            {
                DeletedApplicationUser deletedUser = new DeletedApplicationUser();
                ...
                deletedUser.IsProfileCompleted = user.IsProfileCompleted;
                deletedUser.LastName = user.LastName;
                deletedUser.PictureURL = user.PictureURL;
                deletedUser.Username = user.UserName;
                deletedUser.UserType = user.UserType;
                deletedUser.VAT = user.VAT;
